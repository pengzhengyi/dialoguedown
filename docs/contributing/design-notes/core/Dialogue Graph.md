...
> [Progression Order](../language/Progression%20Order.md) already fixed — succession, divert, the
...
- **Block controls** (`if`/`elseif`/`else`) — lowered here to a **Branch** node whose
  ...
  itself is the [Block Controls](../language/Block%20Controls.md) note.
- **Cross-file links** — reserved by letting a `NodeId` widen to a project-qualified id;
  a file grouping arrives with it. Resolution is the [Cross-File Jump Resolution](../language/Cross-File%20Jump%20Resolution.md) note.
...
| **Succession** | The default edge: fall-through to the next block in document order (see [Progression Order](../language/Progression%20Order.md)). |
...
- **Cross-file node ids.** `NodeId` widens to a project-qualified id for cross-file
  diverts; the [Cross-File Jump Resolution](../language/Cross-File%20Jump%20Resolution.md) linker owns resolution.
