...
- [ ] Records that compare by value across every collection property, generated by
      `Generator.Equals` and guarded by its analyzer.
...
runtime — see [Target Frameworks](../other/Target%20Frameworks.md). It takes two
packages. `System.Text.Json`, scoped to `net8.0`, supplies an attribute the
...
`net8.0` goes. `Generator.Equals` implements the records' value equality (see
[P12](#p12--records-compare-by-value)), and its small runtime travels with it.
...
| Test                           | Asserts                                                                          |
|--------------------------------|----------------------------------------------------------------------------------|
| `Playbook_DependsOnlyOn_TheFrameworkAndGenerator` | `DialogueDown.Playbook` references no other project and, outside `System`, only `Generator.Equals.Runtime` |
...
allows exactly that assembly. Hand-writing `Equals` and `GetHashCode` per record
...
| Central package management | `DialogueDown.Playbook` inherits `Directory.Packages.props` and takes two packages: `System.Text.Json`, scoped to `net8.0`, for an attribute the framework only gained in .NET 9, and `Generator.Equals` for value equality                                                                       |
...
| Equality      | Every record with a collection property compares by value; the `Generator.Equals` analyzer fails the build when one is unannotated |
