...
> [!NOTE]
> Status: **implemented**. The pass that teaches the runner when to keep going
> and when to stop: a step runs on only while the run has
...
Arriving is a **walk**: it visits a node, collects what being there asks for, and
carries on only while the node has asked the host for nothing. It stops at the
first node that asks for something, and that node is where the run stands.
