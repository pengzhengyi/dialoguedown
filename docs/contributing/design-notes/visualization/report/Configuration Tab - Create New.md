...
> [File Launcher](../session/Live%20Visualization%20-%20File%20Launcher.md)'s create-file flow.
...
- **Confinement** reuses the launcher's `BrowseRoot`/serve-root machinery: the target
  ...
  request-supplied string, so the [path-injection](../session/Live%20Visualization%20-%20File%20Launcher.md)
  barrier holds.
...
- **Security.** The path is composed **server-side** from the known serve root
  ...
  client sends no path at all, so the [path-injection](../session/Live%20Visualization%20-%20File%20Launcher.md)
