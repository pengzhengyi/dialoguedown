...
/// <summary>
/// One arm of a <see cref="Nodes.BranchNode"/>: the body it leads to, its <see cref="Guard"/> —
/// an <c>if</c> or <c>elseif</c> carries one; the <c>else</c> arm's is null — and the
/// <see cref="Order"/> it is tried in. Only the first arm whose guard holds is taken, so unlike
...
/// </summary>
internal sealed record BranchEdge(NodeId Target, int Order, Condition? Guard = null)
