...
/// precedes or the line it fronts; one that guards neither — left over in speech, or with no
/// content after it — cannot do anything, so it is an error. The condition keeps its span, so the
/// diagnostic points at the code span itself. A condition is <em>bound</em> when it is exactly the
/// <see cref="Condition"/> its parent jump or line references, so a stray condition sharing a line
/// with a real guard is still caught by identity rather than by its parent's type alone.
/// </summary>
internal sealed class OrphanConditionRule : DiagnosticRule
{
    ...
    // parent is a guarding kind, since a line owns both its guard and its speech.
